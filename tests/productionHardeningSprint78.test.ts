...
test('customer workflows use safe diagnostics and resilient checkout networking', () => {
  ...
  assert.match(cart, /const paymentMethod = 'cod' as const/);
  assert.doesNotMatch(cart, /submitPayHerePayment/);
...
test('price-change cart refresh removes unavailable products instead of preserving a false quantity', () => {
  assert.match(app, /!isProductExplicitlyActive\(refreshed\.isActive\) \|\| refreshed\.stock <= 0\) \{/);
