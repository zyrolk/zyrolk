...
test('manual products are explicitly internal while supplier review controls stay private', () => {
  ...
  const costPriceLabel = 'Cost Price <span className="font-normal text-slate-400">(Supplier-managed, read-only)</span>';
  const costPriceStart = review.indexOf(costPriceLabel);
  const costPriceBlockStart = review.lastIndexOf('<div', costPriceStart);
  const marketPriceBoundaryStart = review.indexOf('Market Price <span className="font-normal text-slate-400">(Supplier reference, read-only)</span>', costPriceStart);
  assert.ok(costPriceStart >= 0 && costPriceBlockStart >= 0 && marketPriceBoundaryStart > costPriceBlockStart);
  const costPriceBlock = review.slice(costPriceBlockStart, marketPriceBoundaryStart);
  assert.match(costPriceBlock, /aria-readonly="true"/);
  assert.match(costPriceBlock, /Supplier-managed, read-only/);
  assert.doesNotMatch(costPriceBlock, /<input/);
  const marketPriceLabel = 'Market Price <span className="font-normal text-slate-400">(Supplier reference, read-only)</span>';
  const marketPriceStart = review.indexOf(marketPriceLabel, costPriceStart);
  const marketPriceBlockStart = review.lastIndexOf('<div', marketPriceStart);
  const stockPriceStart = review.indexOf('Stock <span className="font-normal text-slate-400">(Supplier-managed, read-only)</span>', marketPriceStart);
  assert.ok(marketPriceStart >= 0 && marketPriceBlockStart >= 0 && stockPriceStart > marketPriceBlockStart);
  const marketPriceBlock = review.slice(marketPriceBlockStart, stockPriceStart);
  assert.match(marketPriceBlock, /aria-readonly="true"/);
  assert.match(marketPriceBlock, /Supplier reference, read-only/);
  assert.doesNotMatch(marketPriceBlock, /<input/);
