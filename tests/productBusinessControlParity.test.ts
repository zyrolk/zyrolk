...
test('manual products are explicitly internal while supplier review controls stay private', () => {
  ...
  const marketPriceBoundaryStart = review.indexOf('Market / Reference Price <span className="font-normal text-slate-400">(Read-only)</span>', costPriceStart);
  ...
  const marketPriceLabel = 'Market / Reference Price <span className="font-normal text-slate-400">(Read-only)</span>';
  ...
  assert.match(marketPriceBlock, /Market \/ Reference Price/);
