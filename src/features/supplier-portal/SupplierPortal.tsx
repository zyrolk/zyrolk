...
import {
  ...
  recordSupplierTracking, saveSupplierProfile, submitSupplierProductRequest, updateSupplierFulfilment,
...
export default function SupplierPortal({ user }: SupplierPortalProps) {
  ...
  const productEditorCloseRef = useRef<HTMLButtonElement>(null);
  const productEditorPreviousFocusRef = useRef<HTMLElement | null>(null);
  const productEditorBusyRef = useRef(false);

  useEffect(() => { productEditorBusyRef.current = busy === 'save-product'; }, [busy]);

  useEffect(() => {
    if (!showProductEditor) return;
    productEditorPreviousFocusRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    const previousOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    const focusTimer = window.setTimeout(() => productEditorCloseRef.current?.focus(), 0);
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && !productEditorBusyRef.current) {
        event.preventDefault();
        setShowProductEditor(false);
        return;
      }
      if (event.key !== 'Tab' || !productEditorRef.current) return;
      const focusable = (Array.from(productEditorRef.current.querySelectorAll(
    ...
    return () => {
      ...
      productEditorPreviousFocusRef.current?.focus();
    };
  }, [showProductEditor]);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <header className="border-b border-slate-200 bg-slate-950 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => void signOut(auth)} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-700 px-3 text-xs font-bold hover:bg-slate-800"><LogOut className="h-4 w-4" aria-hidden="true" /> Sign out</button>
          </div>
        </div>
      </header>

      <nav className="sticky top-0 z-20 border-b border-slate-200 bg-white" aria-label="Supplier Hub sections">
        <div className="mx-auto flex max-w-7xl flex-wrap gap-1 px-3 py-2 sm:px-6">
          {tabs.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setTab(id)} aria-current={tab === id ? 'page' : undefined} className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-xs font-bold ${tab === id ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><Icon className="h-4 w-4" aria-hidden="true" />{label}</button>)}
        </div>
      </nav>
      ...
      <main id="supplier-main" tabIndex={-1} className="mx-auto max-w-7xl px-4 py-6 outline-none sm:px-6 sm:py-8">
        ...
        {data && tab === 'products' && <section aria-labelledby="supplier-products-title">
          ...
          <div className="space-y-6">
            <div><h3 className="mb-3 text-sm font-black">Approval Requests</h3>{data.requests.length ? <div className="grid gap-3">{data.requests.map((request) => <article key={request.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h4 className="font-black">{request.productName || 'Untitled draft'}</h4><StatusPill status={request.status} /></div><p className="mt-1 text-xs text-slate-500">SKU {request.supplierSku || 'Not set'} · {request.requestType.replaceAll('_', ' ')}</p>{request.rejectionReason && <p className="mt-2 rounded-lg bg-red-50 p-2 text-xs font-semibold text-red-700">{request.rejectionReason}</p>}</div><div className="flex flex-wrap gap-2">{request.status === 'draft' && <><button type="button" onClick={() => openDraftRequest(request)} className="inline-flex min-h-11 items-center gap-1 rounded-xl border border-slate-200 px-3 text-xs font-bold"><Edit3 className="h-3.5 w-3.5" aria-hidden="true" />Edit</button><button type="button" onClick={() => void runAction(`submit-${request.id}`, () => submitSupplierProductRequest(user, request.id).then(() => undefined), 'Product submitted for admin approval.')} disabled={busy === `submit-${request.id}`} className="inline-flex min-h-11 items-center gap-1 rounded-xl bg-emerald-600 px-3 text-xs font-bold text-white disabled:opacity-50"><Send className="h-3.5 w-3.5" aria-hidden="true" />Submit</button></>}</div></article>)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">No product requests yet.</div>}</div>
            <div><h3 className="mb-3 text-sm font-black">Approved Products</h3>{data.products.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{data.products.map((product) => <article key={product.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><img src={product.imageUrl} alt="" loading="lazy" decoding="async" className="h-40 w-full object-cover" /><div className="p-4"><div className="flex justify-between gap-3"><h4 className="font-black">{product.name}</h4>{product.stock <= product.lowStockLimit && <AlertTriangle className="h-5 w-5 shrink-0 text-amber-500" aria-label="Low stock" />}</div><p className="mt-1 text-xs text-slate-500">Stock {product.stock} · {formatCurrency(product.price)}</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => openProductChange(product)} className="min-h-11 rounded-xl border border-slate-200 px-3 text-xs font-bold">Request Changes</button><input aria-label={`Proposed stock for ${product.name}`} inputMode="numeric" value={stockDrafts[product.id] ?? String(product.stock)} onChange={(event) => setStockDrafts((current) => ({ ...current, [product.id]: event.target.value }))} className="min-h-11 w-20 rounded-xl border border-slate-200 px-2 text-xs" /><button type="button" onClick={() => void runAction(`stock-${product.id}`, () => proposeSupplierStock(user, product.id, Number(stockDrafts[product.id] ?? product.stock)).then(() => undefined), 'Stock proposal sent for review.')} disabled={busy === `stock-${product.id}`} className="min-h-11 rounded-xl bg-slate-900 px-3 text-xs font-bold text-white disabled:opacity-50">Propose Stock</button></div></div></article>)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">No approved products are assigned to this supplier account.</div>}</div>
          </div>
        </section>}

        {data && tab === 'orders' && <section aria-labelledby="supplier-orders-title"><div className="mb-6"><p className="text-xs font-black uppercase tracking-wider text-blue-600">Assigned fulfilment</p><h2 id="supplier-orders-title" className="mt-1 text-2xl font-black">Orders</h2></div>{data.orders.length ? <div className="space-y-4">{data.orders.map((order) => <OrderCard key={order.id} order={order} busy={busy} onStatus={(status) => runAction(`order-${order.id}-${order.groupId}`, () => updateSupplierFulfilment(user, order, status, status === 'unassigned' ? 'Supplier declined the assigned fulfilment group.' : undefined).then(() => undefined), `Order ${order.orderNumber} moved to ${status}.`)} onTracking={(courierName, trackingNumber) => runAction(`tracking-${order.id}-${order.groupId}`, () => recordSupplierTracking(user, order, courierName, trackingNumber).then(() => undefined), `Order ${order.orderNumber} marked as shipped.`)} />)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">No orders are assigned to your supplier account.</div>}</section>}
      ...
      {showProductEditor && data && <div ref={productEditorRef} className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/75 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="supplier-product-editor-title" aria-busy={busy === 'save-product'}><div className="mx-auto max-w-4xl rounded-3xl bg-white p-4 shadow-2xl sm:p-7"><div className="mb-5 flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-blue-600">{requestType === 'new_product' ? 'New product request' : 'Approved product change request'}</p><h2 id="supplier-product-editor-title" className="mt-1 text-xl font-black">Product Editor</h2><p className="mt-1 text-xs text-slate-500">Saving creates a draft only. Publishing always requires administrator approval.</p></div><button ref={productEditorCloseRef} type="button" onClick={() => setShowProductEditor(false)} disabled={busy === 'save-product'} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200 disabled:opacity-50" aria-label="Close product editor"><XCircle className="h-5 w-5" aria-hidden="true" /></button></div><form className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void saveDraft(); }}>
...
function OrderCard({
  ...
  onTracking(courierName: string, trackingNumber: string): Promise<void>;
}) {
  const [courierName, setCourierName] = useState('');
  const [trackingNumber, setTrackingNumber] = useState('');
  const trackingKey = `tracking-${order.id}-${order.groupId}`;
  return <div className="space-y-3">
    <OrderStatusCard order={order} busy={busy} onStatus={onStatus} />
    {order.attributionAvailable && order.supplierFulfilmentStatus === 'packed' && !['cancelled', 'delivered'].includes(order.status) && !order.tracking && <form onSubmit={(event) => { event.preventDefault(); void onTracking(courierName, trackingNumber); }} className="grid gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 sm:grid-cols-2">
      <label className="text-xs font-bold text-slate-700">Courier name<input required maxLength={80} value={courierName} onChange={(event) => setCourierName(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm" /></label>
      <label className="text-xs font-bold text-slate-700">Tracking number<input required maxLength={120} value={trackingNumber} onChange={(event) => setTrackingNumber(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm" /></label>
      <button type="submit" disabled={busy === trackingKey} className="min-h-11 rounded-xl bg-blue-600 px-4 text-xs font-black text-white disabled:opacity-50 sm:col-span-2">{busy === trackingKey ? 'Recording shipment...' : 'Mark as Shipped'}</button>
    </form>}
    {order.tracking && <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm"><p className="text-xs font-black uppercase text-emerald-700">Shipment tracking</p><p className="mt-2"><strong>{order.tracking.courierName}</strong></p><p className="break-all font-mono text-xs">{order.tracking.trackingNumber}</p>{order.tracking.trackingUrl && <a href={order.tracking.trackingUrl} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex text-xs font-bold text-blue-700">Track package</a>}<p className="mt-2 text-[10px] text-slate-500">Shipped {formatDate(order.tracking.recordedAt)}</p></div>}
  </div>;
}

function OrderStatusCard({ order, busy, onStatus }: { order: SupplierPortalOrder; busy: string; onStatus(status: string): Promise<void> }) {
  const nextStatus: Record<string, string | undefined> = { assigned: 'accepted', accepted: 'processing', processing: 'packed' };
  const next = nextStatus[order.supplierFulfilmentStatus];
  const locked = ['cancelled', 'delivered'].includes(order.status);
  const operationKey = `order-${order.id}-${order.groupId}`;
  return <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-black">{order.orderNumber}</h3><StatusPill status={order.attributionAvailable ? order.supplierFulfilmentStatus : 'legacy attribution unavailable'} /></div><p className="mt-1 text-xs text-slate-500">Placed {formatDate(order.createdAt)} · {formatCurrency(order.supplierTotal)}</p></div><div className="flex flex-wrap gap-2">{order.attributionAvailable && order.supplierFulfilmentStatus === 'assigned' && !locked && <button type="button" onClick={() => void onStatus('unassigned')} disabled={busy === operationKey} className="min-h-11 rounded-xl border border-red-200 px-4 text-xs font-black text-red-700 disabled:opacity-50">Decline</button>}{order.attributionAvailable && next && !locked && <button type="button" onClick={() => void onStatus(next)} disabled={busy === operationKey} className="min-h-11 rounded-xl bg-blue-600 px-4 text-xs font-black text-white disabled:opacity-50">{next === 'accepted' ? 'Accept group' : `Mark ${next}`}</button>}</div></div><details className="mt-4 rounded-xl bg-slate-50 p-4"><summary className="flex min-h-11 cursor-pointer items-center text-xs font-black">View order details</summary><div className="mt-4 grid gap-4 text-sm sm:grid-cols-2"><div><h4 className="text-xs font-black uppercase text-slate-500">Delivery</h4><p className="mt-2 font-semibold">{order.customerName}</p><p>{order.customerPhone}</p><p>{order.customerAddress}, {order.city}, {order.district}</p></div><div><h4 className="text-xs font-black uppercase text-slate-500">Payment</h4><p className="mt-2">{order.paymentMethod.replaceAll('_', ' ')}</p><p>{order.paymentStatus.replaceAll('_', ' ')}</p></div><div className="sm:col-span-2"><h4 className="text-xs font-black uppercase text-slate-500">Assigned items</h4><ul className="mt-2 divide-y divide-slate-200">{order.items.map((item) => <li key={`${item.productId}-${item.name}`} className="flex justify-between gap-3 py-2"><span>{item.name} × {item.quantity}</span><strong>{formatCurrency(item.price * item.quantity)}</strong></li>)}</ul></div></div></details></article>;
}
