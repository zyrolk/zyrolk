  ...
  return (
    <div className="zy-order-details" id="customer-order-invoice">
      ...
      {selectedOrder.paymentMethod === 'payhere' && <section className="zy-order-payment-timeline" aria-labelledby="payment-timeline-title">
        ...
        {selectedOrder.paymentReference && <p>PayHere transaction reference <strong>{selectedOrder.paymentReference}</strong></p>}
      </section>}
      ...
      <div className="zy-order-detail-grid">
        ...
        <section className="zy-account-form-card zy-order-contact-card"><div className="zy-account-panel-heading"><div><small>Payment</small><h3>Payment method</h3></div><ShieldCheck /></div><strong className="zy-order-payment-method">{getPaymentMethodLabel(selectedOrder.paymentMethod)}</strong><p>{selectedOrder.paymentMethod === 'payhere' ? `Status: ${(selectedOrder.paymentStatus || 'pending').replace(/_/gu, ' ')}${selectedOrder.paymentAttempt ? ` · Attempt ${selectedOrder.paymentAttempt}` : ''}` : 'Payment and checkout processing remain protected by the existing Zyro.lk commerce flow.'}</p></section>
