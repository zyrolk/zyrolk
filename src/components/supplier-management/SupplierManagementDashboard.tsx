  ...
  if (loading) {
    return (
      <section aria-labelledby="supplier-management-summary-title" className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6" aria-label="Loading supplier dashboard">{cards.map(([label]) => <div key={label} className="h-24 animate-pulse rounded-2xl bg-slate-100 dark:bg-slate-800" />)}</div>
      </section>
    );
  }
