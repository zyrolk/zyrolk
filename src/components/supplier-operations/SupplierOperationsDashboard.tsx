...
function SupplierOperationsDashboard({
  ...
  const updateAlertStatus = async (alertId: string, status: 'acknowledged' | 'resolved') => {
    ...
    setActionError(null);
    try {
      await readJson(await requestApi(`/api/supplier-operations/alerts/${encodeURIComponent(alertId)}/action`, 'POST', { status }));
      await loadAll(true);
    } catch (actionError) {
      setActionError(actionError instanceof Error ? actionError.message : 'Alert status could not be updated.');
    } finally {
      setActionId(null);
    }
  };
  ...
  return (
    <div className="space-y-8" data-testid="supplier-operations-dashboard">
      ...
      {mode === 'advanced' && <section aria-labelledby="alerts-title" className="rounded-3xl border border-slate-200/70 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
        ...
        {snapshot?.alerts.length ? <div className="grid gap-3 lg:grid-cols-2">{snapshot.alerts.map((alert) => <div key={alert.id} className="rounded-2xl border border-amber-200/70 bg-amber-50/70 p-4 dark:border-amber-900/50 dark:bg-amber-950/20"><div className="flex items-center justify-between gap-2"><strong className="text-sm text-slate-900 dark:text-white">{alert.title}</strong><span className="text-[9px] font-black uppercase text-amber-700">{alert.severity}</span></div><p className="mt-1 text-xs text-slate-600 dark:text-slate-300">{alert.message}</p>{alert.status && <div className="mt-3 flex flex-wrap items-center gap-2"><span className="text-[9px] font-black uppercase text-slate-500">{stateLabel(alert.status)}</span>{alert.status === 'open' && <button type="button" onClick={() => void updateAlertStatus(alert.id, 'acknowledged')} disabled={actionId === `${alert.id}:acknowledged`} className="min-h-9 rounded-lg border border-amber-300 px-3 text-[10px] font-black text-amber-800 disabled:opacity-50">Acknowledge</button>}<button type="button" onClick={() => void updateAlertStatus(alert.id, 'resolved')} disabled={actionId === `${alert.id}:resolved`} className="min-h-9 rounded-lg bg-slate-900 px-3 text-[10px] font-black text-white disabled:opacity-50">Resolve</button></div>}</div>)}</div> : <p className="text-sm text-slate-500">No active alerts.</p>}
      </section>}
