...
export function buildA2ZProductImageUrl(productId: unknown): string | null {
  ...
  if (!/^\d+$/.test(normalizedId) || normalizedId === '0') return null;
  return `${A2Z_PRODUCT_IMAGE_ORIGIN}/storage/products/a2z/${normalizedId}.jpg`;
}

export function normalizeSupplierProductImages(imageUrl: unknown, imageUrls: unknown): string[] {
  const candidates = [
    imageUrl,
    ...(Array.isArray(imageUrls) ? imageUrls : []),
  ];

  return [...new Set(candidates.filter(isValidSupplierImageUrl).map((value) => value.trim()))];
}
